...
async function main() {
  ...
  if (CITY) {
    ...
    // Full JSON (used by server / S3 later)
    const json = JSON.stringify(output, null, 2);
    fs.writeFileSync(tsFile,  json, "utf8");
    fs.writeFileSync(latFile, json, "utf8");

    // Slim JS (used by index.html directly via <script src>)
  ...
  // ── Flat run: save jobs.json + jobs.js as before ──────────────────────────
    ...
    if (S3_BUCKET) {
      ...
      const uploads = [
        { key: `${S3_PREFIX}/jobs.json`, body: JSON.stringify(output, null, 2), type: "application/json" },
