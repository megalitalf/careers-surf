 ...
 *     → geocodes Warsaw, scrapes with that location filter, saves:
 *         cities/warsaw/<timestamp>.json
 *         cities/warsaw/latest.json
 *
 * Location flags:
 ...
 *
 * City-run flags:
 *   --city <name>               Named city run — implies --location, routes output to cities/<slug>/
 *   --cities-dir <path>         Root dir for city runs (default: cities/)
 *
 * Config (edit below):
...
const CITY         = getArg("--city", "");   // named city run
...
const LOCATION     = _locArg  || CITY || _config.location?.query  || "";
...
async function main() {
  ...
  // ── City run: save to cities/<slug>/<timestamp>.json + latest.json ─────────
  if (CITY) {
    const slug      = toSlug(CITY);
    const cityDir   = path.join(CITIES_DIR, slug);
    const timestamp = output.meta.scrapedAt.replace(/[:.]/g, "-").replace("Z", "z"); // filesystem-safe
    ...
    // Slim listings — shared by both .json and .js outputs
  ...
  // ── Flat run: save jobs.json + jobs.js as before ──────────────────────────
    ...
    // JS (used by index.html directly without a server)
    ...
    // ── Upload flat files to S3 if S3_BUCKET is set ─────────────────────────
    if (S3_BUCKET) {
      ...
      for (const { key, body, type } of uploads) {
        ...
        try {
          await s3.send(new PutObjectCommand({
            Bucket: S3_BUCKET, Key: key, Body: body, ContentType: type, CacheControl: "max-age=300",
          }));
          console.log("✅");
        } catch (err) {
          console.log(`❌  ${err.message}`);
